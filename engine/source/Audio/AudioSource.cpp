...
AudioSource::AudioSource(const AudioSource &audioSource) : m_volume(audioSource.m_volume), m_id(audioSource.m_id), m_groupName(audioSource.m_groupName), m_fileName(audioSource.m_fileName), m_sfInfo(audioSource.m_sfInfo) {
  if (!(m_fileData = sf_open(m_fileName.c_str(), SFM_READ, &m_sfInfo))) {
    sf_perror(m_fileData);
    throw std::runtime_error{"AudioSource::AudioSource => sf_open(m_fileName.c_str(), SFM_READ, &m_sfInfo) returned error"};
  }
