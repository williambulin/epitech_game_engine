...
public:
  ...
  [[nodiscard]] static float SqDistPointAABB(const Vec3 &p, const AABB &b ) noexcept {
    ...
    if( p.x > b.m_topRight.x ) sqDist += (p.x - b.m_topRight.x) * (p.x - b.m_topRight.x);
    if( p.y < b.m_bottomLeft.y ) sqDist += (b.m_bottomLeft.y - p.y) * (b.m_bottomLeft.y - p.y);
    if( p.y > b.m_topRight.y ) sqDist += (p.y - b.m_topRight.y) * (p.y - b.m_topRight.y);
    if( p.z < b.m_bottomLeft.z ) sqDist += (b.m_bottomLeft.z - p.z) * (b.m_bottomLeft.z - p.z);
    if( p.z > b.m_topRight.z ) sqDist += (p.z - b.m_topRight.z) * (p.z - b.m_topRight.z);
